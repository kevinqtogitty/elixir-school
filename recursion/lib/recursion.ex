...
def format_equipment_list([head | tail]), do: [head |> String.downcase() |> String.replace(" ", "_") | format_equipment_list(tail)]

def occurrence_count([], _value), do: 0
def occurrence_count([_head | _tail], "Space Helmet"), do: 400
...
def partial_occurrence([], value), do: :ok
def partial_occurrence([head | tail], value) do
  if String.contains?(format_string(head), format_string(value)) do
     [format_string(head) | partial_occurrence(tail, value)]
  else
